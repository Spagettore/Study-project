...
public class PrintRoomHistoryAction implements IAction {
    ...
    public void execute() {
        ...
        int number = InputHelper.enterNumber("Введите количество логов в истории заселения комнаты (0 для вывода всех, -1 для значения из конфига): ", false);
        ...
        try {
            ...
            if (number < 0) {
                OutputHelper.printRoomHistory(Hotel.getInstance().getRoomHistory(id, Hotel.getInstance().getConfig().getRoomHistoryCount()));
                return;
            }
